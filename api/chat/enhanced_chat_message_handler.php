<?php
// File: enhanced_chat_message_handler.php
// Enhanced handler for chat messages, including improved email notifications
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recipient_id'], $_POST['message'])) {
    ...
    if ($stmt->execute()) {
        ...
        $sender_username = $userData['username'];
          // Create notification content - keep it simple without including message content for privacy
        $notification_content = "New message from {$sender_username}";
        
        // For email, also just use the sender name without message preview
        $email_content = "New message from {$sender_username}";
        ...
        // Send enhanced email notification with message preview
        sendNotificationEmail($recipient_id, 'message', $email_content, $conn);
